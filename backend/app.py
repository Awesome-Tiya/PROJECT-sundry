...
def genqr():
    ...
    return jsonify({
        "message": "QR Code saved as 'siteqr.png'",
        "image_url": "/static/siteqr.png"
    })
...
def books():
    ...
    try:
      ...
      url='https://www.goodreads.com/blog/show/2687-goodreads-members-76-most-popular-books-of-the-past-decade?ref=decadepopular2023_eb'
    ...
    if(len(books) < 1): 
       return jsonify({ "genre":"This genre is not valid. Enter correct genre.", 
                        "books": books }) 
...
def color_effect():
    ...
    if "1"==col or "green"==col.lower():
      plt.subplot(1, 1, 1)
      window_name = 'image'
      cv2.imshow(window_name, green_demo) #this line is commented because it does not open a window at vs code codespace. It can be uncommented in an environment where a window can be opened to preview the image file
      ...
      plt.subplot(1, 1, 1)
      window_name = 'image'
      cv2.imshow(window_name, blue_demo) #this line is commented because it does not open a window at vs code codespace. It can be uncommented in an environment where a window can be opened to preview the image file
      ...
      plt.subplot(1, 1, 1)
      window_name = 'image'
      cv2.imshow(window_name, grey_demo) #this line is commented because it does not open a window at vs code codespace. It can be uncommented in an environment where a window can be opened to preview the image file
      ...
      plt.subplot(1, 1, 1)
      window_name = 'image'
      cv2.imshow(window_name, purple_demo) #this line is commented because it does not open a window at vs code codespace. It can be uncommented in an environment where a window can be opened to preview the image file
